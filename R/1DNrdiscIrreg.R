# simple one dimensional regression example, without spatial correlation, with nugget, and a 
...
for (Rsel in 1:R) {
  ...
  # hper <- as.matrix( dist( sper))
  # yR <- rmvnorm(1, mu=Xper%*%beta0, Sigma=diag(theta0, nrow=n))
  # 
  # y <- c(yR)
  
  yR <- rmvnorm(1, Sigma=diag(theta0, nrow=n))
  ...
  outper = lm(y ~ Xper-1)
  outapp = lm(y ~ Xapp-1)
  llper = logLikMVN(y - Xper %*% outper$coefficients, chol(diag(summary(outper)$sigma, nrow=n)))
  llapp = logLikMVN(y - Xapp %*% outapp$coefficients, chol(diag(summary(outapp)$sigma, nrow=n)))
  llappTrue = logLikMVN(y - Xper %*% outapp$coefficients, chol(diag(summary(outapp)$sigma, nrow=n)))
  
  parResult[Rsel,] <- c(outper$coefficients, -2*llper, outapp$coefficients,-2*llapp, 
                        -2*llappTrue)
...
colnames(parResult) = c("beta0per", "beta1per", "2nllper", 
                        "beta0app", "beta1app", "2nllapp", 
...
diff3 <- parResult[,1:3]- parResult[,4:6] # good pars and 2nll - bad pars and 2nll
truePar = c(beta0)
biasper = sweep(parResult[,1:2], 2, truePar)
biasapp = sweep(parResult[,4:5], 2, truePar)

# pair plot of good model estimates
pairs( parResult[,c(1:7, 15)], gap=0, pch=19, cex=.4)
...
relBiasper = 100*(parResult[,2] - truePar[-1])/truePar[-1]
relBiasapp = 100*(parResult[,5] - truePar[-1])/truePar[-1]
relBiasper = as.data.frame(cbind(relBiasper, estPctBiasBeta1=100*(parResult[,14]-1), misclassPct=100*(parResult[,15]-1)))
relBiasapp = as.data.frame(cbind(relBiasapp, estPctBiasBeta1=100*(parResult[,14]-1), misclassPct=100*(parResult[,15]-1)))
colMeans(parResult[,1:7])
